#!/usr/bin/env python3
"""
Qwen3 TTS — Server for Native App
==================================
Used by app_native.py (pywebview). Same as server_minimal.py
but with /api/health and /api/shutdown endpoints.

The UI has a "Quit" button that calls pywebview.api.shutdown()
to close the native window cleanly.
"""
...
function quitApp() {
  // Try pywebview native API first, fallback to HTTP
...
// Show audio on page load (from form post)
...
def generate():
    if not _model_ready.is_set():
        return render_template_string(HTML.replace('style="display:none" id="output"','id="output"').replace('<div id="output" class="card" style="display:none">','<div id="output" class="card"><p style="color:#f87171">⚠️ Load a model first!</p>'))
...
def serve_output(fname):
    return send_file(os.path.join(OUTPUT_DIR, os.path.basename(fname)), mimetype="audio/wav")
...
if __name__ == "__main__":
    ...
    print(f"\n🎙️  Qwen3 TTS — Server running on http://localhost:8765")
